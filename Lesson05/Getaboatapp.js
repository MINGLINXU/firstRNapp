...
const Getaboatapp = () => {
  ...
  for (let i = 0; i < PRODUCTS_DATA.length; i++) {
    ...
    productsArr.push(<Text key={product.name}>{product.name}</Text>);
    productsArr.push(<Text key={product.description}>{product.description}</Text>);
    productsArr.push(<Image source={product.picture} />);
  }
  return (
    <ScrollView>
      <Text>GetABoat - For Sale</Text>
      <View>{productsArr}</View>
    </ScrollView>
  );
};
